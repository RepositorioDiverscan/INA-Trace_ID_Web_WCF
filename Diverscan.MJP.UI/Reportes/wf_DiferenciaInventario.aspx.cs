...
{
    public partial class wf_DiferenciaInventario : System.Web.UI.Page
    {
        ...
        protected void RadGrid_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
        {
            try
            {
                if (btnGenera.Text.Equals("Reset"))
                {
                    var IdArticulo = DdlIdarticuloInterno.SelectedValue.ToString();
                    var idCompania = n_WMS.ObtenerCompaniaXUsuario(UsrLogged.IdUsuario);
                    var OpcionReporte = rdbTodos.Checked ? "Todos" : "Diferentes";
                    var DiferenciaInventarioList = n_DiferenciaInventario.ObtenerReporteDiferenciaInventario(IdArticulo, OpcionReporte, idCompania);

                    if (DiferenciaInventarioList.Count > 0)
                    {
                        rdDatosReporteDiferenciaInventario.DataSource = DiferenciaInventarioList;
                        //rdDatosReporteDiferenciaInventario.DataBind();
                    }
                }
                else
                {
                    rdDatosReporteDiferenciaInventario.DataSource = dataDiferenciaInventario;
                }
            }
            catch (Exception)
            {
        ...
        private void CargarDDLS()
        {
            try
            {
              ...
              */
                DdlIdarticuloInterno.Items.Insert(1, new ListItem("Todos"));
            }
            catch (Exception ex)
            {
        ...
        protected void btnGenera_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (DdlIdarticuloInterno.SelectedIndex == 0)
                {
                ...
                if (seleccionoArticulo)
                {
                    if (btnGenera.Text == "Vista Previa")
                    {
                        ...
                        IdArticulo = DdlIdarticuloInterno.SelectedValue.ToString();
                        idCompania = n_WMS.ObtenerCompaniaXUsuario(UsrLogged.IdUsuario);
                        OpcionReporte = rdbTodos.Checked ? "Todos":"Diferentes";
                        var DiferenciaInventarioList = n_DiferenciaInventario.ObtenerReporteDiferenciaInventario(IdArticulo, OpcionReporte, idCompania);
                        ...
                        if (DiferenciaInventarioList.Count > 0)
                        {
                            ...
                            //gvDiferenciaInventario.DataSource = DiferenciaInventarioList;
                            //gvDiferenciaInventario.DataBind();

                            BtnExporta.Enabled = true;
                            btnGenera.Text = "Reset";
                            DdlIdarticuloInterno.Enabled = false;
                            rdbTodos.Enabled = false;
                            rdbDiferencias.Enabled = false;
                            //dataDiferenciaInventario = DiferenciaInventarioList;
                        }
                    }
                    else
                    {
                        rdDatosReporteDiferenciaInventario.DataSource = null;
                        rdDatosReporteDiferenciaInventario.DataBind();
                        ...
                        DdlIdarticuloInterno.Enabled = true;
                        rdbTodos.Enabled = true;
                        rdbDiferencias.Enabled = true;
                    }
        ...
        protected void BtnExporta_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string IdArticulo = "";
                    string idCompania = n_WMS.ObtenerCompaniaXUsuario(UsrLogged.IdUsuario);
                    string OpcionReporte = rdbTodos.Checked ? "Todos" : "Diferentes";
                    DataSet DSDatos = new DataSet();
                    IdArticulo = DdlIdarticuloInterno.SelectedValue.ToString();

                    string SQL = "exec Obtener_DiferenciaInventarioExport @IdArticulo=N'" + IdArticulo + "', @OpcionReporte=N'" + OpcionReporte + "', @IdCompania=N'" + idCompania + "'";

                    DSDatos = n_ConsultaDummy.GetDataSet(SQL, "");

                    DataRow dr = DSDatos.Tables[0].NewRow();
                    dr[0] = "Codigo Interno";
                    dr[1] = "Nombre Articulo";
                    dr[2] = "Disponile Inventario";
                    dr[3] = "Disponile ERP";
                    dr[4] = "Diferencia ERP-Inventario";
                    dr[5] = "Unidad Medida";

                    DSDatos.Tables[0].Rows.InsertAt(dr, 0);

                    var rutaVirtual = "~/temp/" + string.Format("Reporte Diferencia Inventario.xlsx");
                    //var rutaVirtual = RutaTemp + string.Format("Reporte Diferencia Inventario.xlsx");
        ...
        private void AplicaParametros()
        {
            ...
            valoresd.Value = int.Parse(DdlIdarticuloInterno.SelectedValue.ToString());  // Toma el valor del parámetro.
            ...
            valoresd.Value = DdlIdarticuloInterno.SelectedValue.ToString() == "--Seleccionar--" ? 0 : int.Parse(DdlIdarticuloInterno.SelectedValue.ToString());  // si no eligió nada, pone 00.
            pfds = Documento.DataDefinition.ParameterFields;   // establece los parámetros del reporte.
        ...
        List<e_DiferenciaInventario> dataDiferenciaInventario
        {
            get
            {
                var data = ViewState["DataDiferenciaInventario"] as List<e_DiferenciaInventario>;
                if (data == null)
                    return new List<e_DiferenciaInventario>();
                return data;
            }
            set
            {
                ViewState["DataDiferenciaInventario"] = value;
            }
        }

        protected void gvDiferenciaInventario_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            //gvDiferenciaInventario.PageIndex = e.NewPageIndex;
            //gvDiferenciaInventario.DataSource = dataDiferenciaInventario;
            //gvDiferenciaInventario.DataBind();
        }

        protected void gvDiferenciaInventario_RowCommand(object sender, GridViewCommandEventArgs e)
        {
