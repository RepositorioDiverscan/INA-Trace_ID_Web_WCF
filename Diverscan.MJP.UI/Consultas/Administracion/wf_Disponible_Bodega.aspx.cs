...
{
    public partial class wf_Disponible_Bodega : System.Web.UI.Page
    {
        ...
        private void ConsultaArticulos(string buscar, bool pestana)
        {
            try
            {
                n_WMS wms = new n_WMS();
                ...
                SQL = "EXEC SP_Consulta_ArticulosDisponibles '" + buscar + "'";
                DSDatos = n_ConsultaDummy.GetDataSet(SQL, UsrLogged.IdUsuario);

                RadGridArticulosDisponibles.DataSource = DSDatos;
                if (pestana)
                {
        ...
        protected void btnBuscar_Click(object sender, EventArgs e)
        {
            try
            {
                ConsultaArticulos(txtSearch.Text.ToString().Trim(), true);
                //txtSearch.Text = "";
            }
            catch (Exception ex)
            {
