...
{
    public partial class wf_ConsultaSSCC : System.Web.UI.Page
    {
        ...
        private void ConsultaSSCC(string buscar, bool pestana)
        {
            try
            {
                n_WMS wms = new n_WMS();
                ...
                SQL = "EXEC SP_Consulta_Contenido_SSCC '"  + buscar + "'";
                DSDatos = n_ConsultaDummy.GetDataSet(SQL, UsrLogged.IdUsuario);

                RadGridSSCC.DataSource = DSDatos;
                if (pestana)
                {
                    RadGridSSCC.DataBind();
                }
            }
            catch (Exception ex)
            {
        ...
        protected void RadGridGTIN14_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
        {
            try
            {
                ConsultaSSCC(txtSearch.Text.ToString().Trim(), false);
                //CargarGTIN14("", false);
            }
            catch (Exception ex)
            {
        ...
        protected void btnBuscar_Click(object sender, EventArgs e)
        {
            try
            {
                ConsultaSSCC(txtSearch.Text.ToString().Trim(), true);
                //txtSearch.Text = "";
            }
            catch (Exception ex)
            {
