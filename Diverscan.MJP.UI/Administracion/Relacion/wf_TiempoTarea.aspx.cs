...
{
    public partial class wf_TiempoTarea : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
             ...
             CargarNombreTablas(e_BaseDatos.NombreBD(),ddlTabla);
        ...
        private void CargarNombreColumnas(DropDownList DDL)
        {
            ...
            {
            DDL.Items.Clear();
            string tabla = "'" + ddlTabla.SelectedValue.ToString().Substring(4) + "'";
            DataSet DSBaseDatos = new DataSet();
            string SQL = "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS  WHERE (TABLE_NAME = " + tabla + ")";
          //  string SQL = "SELECT name FROM Sys.Databases WHERE  ORDER BY name ASC";
            DSBaseDatos = n_ConsultaDummy.GetDataSet(SQL, "0");
            if (DSBaseDatos != null)
            {
                if (DSBaseDatos.Tables[0].Rows.Count > 0)
        ...
        private void CargarDDLS()
        {
            try
            {
                string[] Msj = n_SmartMaintenance.CargarDDL(ddlidMetodoAccion, e_TablasBaseDatos.TblMetodoAccion(), UsrLogged.IdUsuario, false);
                if (Msj[1] != "") Mensaje(Msj[0], Msj[1], "");
                //Msj = n_SmartMaintenance.CargarDDL(ddlIdCompania, e_TablasBaseDatos.TblCompania(), UsrLogged.IdUsuario);
                //if (Msj[1] != "") Mensaje(Msj[0], Msj[1], "");
                //Msj = n_SmartMaintenance.CargarDDL(ddlCampoWMS, e_TablasBaseDatos.VistaCampoWMS(), UsrLogged.IdUsuario);
                //if (Msj[1] != "") Mensaje(Msj[0], Msj[1], "");
                //Msj = n_SmartMaintenance.CargarDDL(ddlCampoERP, e_TablasBaseDatos.VistaCampoERP(), UsrLogged.IdUsuario);
                //if (Msj[1] != "") Mensaje(Msj[0], Msj[1], "");
        ...
        protected void btnAgregar_Click(object sender, EventArgs e)
        {
            Control Ctr = (Control)sender;
            var Panel = Ctr.Parent.Parent.Parent;
            string[] Msj = n_SmartMaintenance.AgregarDatos(Panel, e_TablasBaseDatos.TblRelAccionTiempo(), ToleranciaAgregar, UsrLogged.IdUsuario);
            if (Msj[1] != "") Mensaje(Msj[0], Msj[1], "");
        }
        ...
        protected void btnEditar_Click(object sender, EventArgs e)
        {
            Control Ctr = (Control)sender;
            var Panel = Ctr.Parent.Parent.Parent;
            string[] Msj = n_SmartMaintenance.EditarDatos(Panel, e_TablasBaseDatos.TblRelAccionTiempo(), UsrLogged.IdUsuario);
            if (Msj[1] != "") Mensaje(Msj[0], Msj[1], "");
        }
        ...
        protected void ddlTabla_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                DataSet DSBaseDatos = new DataSet();
                txtPKObjeto.Text = "";
                ddlPkValue.Items.Clear(); 
                string NombreTabla = ddlTabla.SelectedItem.Value.ToString();
                NombreTabla = NombreTabla.Substring(4);
                string SQL = "SELECT Col.Column_Name from  INFORMATION_SCHEMA.TABLE_CONSTRAINTS Tab, INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE Col WHERE Col.Constraint_Name = Tab.Constraint_Name AND Col.Table_Name = Tab.Table_Name AND Constraint_Type = 'PRIMARY KEY' AND Col.Table_Name = '" + NombreTabla + "'";
                string valor = n_ConsultaDummy.GetUniqueValue(SQL, "0");
                string SQL1 = "SELECT " + valor + " FROM " + NombreTabla;              
                DSBaseDatos = n_ConsultaDummy.GetDataSet(SQL1, "0");
                if (DSBaseDatos.Tables[0].Rows.Count > 0)
                {
                    foreach (DataRow dsRowEvento in DSBaseDatos.Tables[0].Rows)
                    {
                        string name = dsRowEvento[valor].ToString();
                        ddlPkValue.Items.Add(name);
                    }
                }

              
                string valor1 = n_ConsultaDummy.GetUniqueValue(SQL1 ,"O");
                txtPKObjeto.Text = valor;
            }
            catch (Exception)
            {
