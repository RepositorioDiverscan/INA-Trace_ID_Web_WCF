...
namespace Diverscan.MJP.UI.Operaciones.Tareas

{
   

    public partial class GeneraTarea : System.Web.UI.Page
    {
        ...
        private void CargarDDLS()
        {
            try
            {
                //string[] Msj = n_SmartMaintenance.CargarDDL(ddlidMDTipoPalabra, e_TablasBaseDatos.TBLMDTipoPalabra(), UsrLogged.IdUsuario);
                //if (Msj[1] != "") Mensaje(Msj[0], Msj[1], "");
                //Msj = n_SmartMaintenance.CargarDDL(ddlidMDContexto, e_TablasBaseDatos.TBLMDContexto(), UsrLogged.IdUsuario);
                //if (Msj[1] != "") Mensaje(Msj[0], Msj[1], "");
        ...
        protected void btnAgregar_Click(object sender, EventArgs e)
        {
            Control Ctr = (Control)sender;
            var Panel = Ctr.Parent.Parent.Parent;
            string[] Msj = n_SmartMaintenance.AgregarDatos(Panel, e_TablasBaseDatos.TBLMDSinonimo(), ToleranciaAgregar, UsrLogged.IdUsuario);
            if (Msj[1] != "") Mensaje(Msj[0], Msj[1], "");
        }
        ...
        protected void btnEditar_Click(object sender, EventArgs e)
        {
            Control Ctr = (Control)sender;
            var Panel = Ctr.Parent.Parent.Parent;
            string[] Msj = n_SmartMaintenance.EditarDatos(Panel, e_TablasBaseDatos.TBLMDSinonimo(), UsrLogged.IdUsuario);
            if (Msj[1] != "") Mensaje(Msj[0], Msj[1], "");
        }
        ...
        protected void SiguienteTarea(object sender, EventArgs e)
        {
            try
            {        
                ////Cambios LuisR
                ImgEspera.Visible = true;
                string Pagina = "~/HH/Operaciones/Tareas/GeneraTarea.aspx";
                string[] respuesta = n_SmartMaintenance.CargarEjecutarAccion(Pagina, "1", UsrLogged.IdUsuario, "btnAccion12").Split(';');
                string idTarea = respuesta[0];
                string Nombre = respuesta[1];
                string Lote = respuesta[2];
                string idUbicacion = respuesta[3];
                string Cantidad = respuesta[4];
                string Estante = respuesta[5];
                string Zona = respuesta[6];
                string Nivel = respuesta[7];
                string Columna = respuesta[8];
                string Posicion = respuesta[9];
                string idartic = respuesta[10];

               txtTarea.Text = idTarea + "\n" + Nombre + "\n" + Lote + "\n" + idUbicacion + "\n" + Cantidad + "\n" + Estante + "\n" + Zona + "\n" + Columna + "\n" + Posicion  + "\n" + idartic ;
            }
            catch (Exception ex)
            {
