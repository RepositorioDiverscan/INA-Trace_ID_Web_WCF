...
{
    public partial class wf_Certificacion : System.Web.UI.Page
    {
        ...
        private void ValidateButtonCertificar()
        {
            if(UsrLogged.IdRoles != "1")
            {
        ...
        /*//SE ELIMINO EL REPORTE DE GENERAR PDF MEDIENTE CRISTAL REPORT. FECHA :30/11/2022
        ...
        protected void btnCertificar_Click(object sender, EventArgs e)
        {
            ...
            var Ola = _listMaestros.Find(x => x.IdMaestroSolicitud == _idOla);
            FileExceptionWriter fileExceptionWriter = new FileExceptionWriter();
            N_Certificacion nCerficacion = new N_Certificacion(fileExceptionWriter);
            long idUsuario = long.Parse(((e_Usuario)Session["USUARIO"]).IdUsuario);
            string result = nCerficacion.CertificarOla(Ola.IdMaestroSolicitud, idUsuario);

            if (result.Contains("Ola"))
            {
                Mensaje("info", result, "");
                SearchOlas();
                return;
