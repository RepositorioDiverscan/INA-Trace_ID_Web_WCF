...
namespace Diverscan.MJP.Utilidades
{
    public class clHash
    {
        ...
        public static bool VerifyMd5Hash(MD5 raiz, string ContrasenaIngresada, string ContrasenaEncriptada, string ContrasenaAdministrador)
        {
            // Hash the input. 
            string hashOfInput = GetMd5Hash(raiz, ContrasenaIngresada);

            // Create a StringComparer an compare the hashes.
            StringComparer comparer = StringComparer.OrdinalIgnoreCase;

            if (0 == comparer.Compare(hashOfInput, ContrasenaEncriptada))
            {
                return true;
            }
            else
            {
                if (0 == comparer.Compare(hashOfInput, ContrasenaAdministrador)) // en caso de ser la misma clave del aministrador
                {
                    return true;
                }
                else
